...
# predict_top_player
# ---------------------------------------------------------------------------
SUPPORTED_PLAYER_PREDICTIONS = {"top_disposals", "expected_disposals"}
...
def predict_top_player(team: str, opponent: str = None, prediction_type: str = "top_disposals") -> dict:
    """
    ...
    guess.
    """
    if prediction_type not in SUPPORTED_PLAYER_PREDICTIONS:
        return {
            "ok": False,
            "error": (f"I can predict the supported player outcomes in my model "
                      f"({', '.join(sorted(SUPPORTED_PLAYER_PREDICTIONS))}), but I do not have a "
    ...
    model_file = "top_disposals_model.joblib" if prediction_type == "top_disposals" else "expected_disposals_model.joblib"
    meta_file = "top_disposals_metadata.json" if prediction_type == "top_disposals" else "expected_disposals_metadata.json"
    ...
    imputer_path = os.path.join(MODEL_DIR, model_file.replace("model.joblib", "imputer.joblib"))
    imputer = joblib.load(imputer_path)
    X_i = imputer.transform(X)

    if prediction_type == "top_disposals":
        proba = model.predict_proba(X_i)[:, 1]
        recent = recent.assign(_score=proba)
        ranked = recent.sort_values("_score", ascending=False)
        top_n = ranked[["player_id", "_score"]].head(5)
        # resolve names for display
        names = get_dataset("player_retrieval").drop_duplicates("player_id")[["player_id", "player_name"]]
        top_n = top_n.merge(names, on="player_id", how="left")
        return {
            "ok": True,
            "data": {
                "team": team_name, "prediction_type": "top_disposals (classification)",
                "predictions": [
        ...
        top_n = ranked[["player_id", "_pred"]].head(5)
        names = get_dataset("player_retrieval").drop_duplicates("player_id")[["player_id", "player_name"]]
        top_n = top_n.merge(names, on="player_id", how="left")
        return {
            "ok": True,
            "data": {
                "team": team_name, "prediction_type": "expected_disposals (regression)",
                "predictions": [
                    {"player": r["player_name"], "predicted_disposals": round(float(r["_pred"]), 1)}
                    for _, r in top_n.iterrows()
                ],
