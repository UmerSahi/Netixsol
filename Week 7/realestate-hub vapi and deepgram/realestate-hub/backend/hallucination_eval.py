...
from config import RESULTS_DIR, get_engine
...
def _get_retriever():
    if "r" not in _retriever_cache:
        from rag_pipeline import get_vectorstore, get_retriever
...
def run_eval(with_guardrails, _generate_fn=None, _judge_fn=None,
    ...
    _generate_fn = _generate_fn or _default_generate
    _judge_fn = _judge_fn or _default_judge
    rows = []
...
if __name__ == "__main__":
    ...
    df1.to_csv(RESULTS_DIR / "hallucination_eval_results.csv", index=False)
    ...
    df2.to_csv(RESULTS_DIR / "hallucination_eval_results_with_guardrails.csv", index=False)
