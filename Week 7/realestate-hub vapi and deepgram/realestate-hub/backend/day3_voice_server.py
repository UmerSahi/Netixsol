...
from config import PROJECT_ROOT, EVALUATIONS_DIR, RECORDINGS_DIR
import websockets
...
def clean_for_speech(text: str) -> str:
    """Normalize Markdown, technical codes, and currency amounts for natural voice delivery."""
    # 1. Remove Markdown bold/italic/headers
    s = re.sub(r"\*\*([^*]+)\*\*", r"\1", text)
    s = re.sub(r"\*([^*]+)\*", r"\1", s)
    s = re.sub(r"#+\s*", "", s)

    # 2. Format property IDs into natural words: PROP-1116 -> Property 1116
    s = re.sub(r"\bPROP-(\d+)\b", r"Property \1", s, flags=re.IGNORECASE)

    # 3. Format PKR and Rupee amounts into natural Pakistani units (Crore / Lakh)
    def format_pkr(match):
        num_str = match.group(1).replace(",", "")
        try:
            val = float(num_str)
            if val >= 10_000_000:
                crore = val / 10_000_000
                return f"{crore:.1f} crore rupees" if crore % 1 != 0 else f"{int(crore)} crore rupees"
            elif val >= 100_000:
                lakh = val / 100_000
                return f"{lakh:.1f} lakh rupees" if lakh % 1 != 0 else f"{int(lakh)} lakh rupees"
            else:
                return f"{int(val):,} rupees"
        except Exception:
            return match.group(0)

    s = re.sub(r"\bPKR\s*([\d,]+(?:\.\d+)?)\b", format_pkr, s, flags=re.IGNORECASE)
    s = re.sub(r"\bRs\.?\s*([\d,]+(?:\.\d+)?)\b", format_pkr, s, flags=re.IGNORECASE)

    # 4. Clean numbered bullet points: "1. " -> "Option 1: "
    s = re.sub(r"(?m)^\s*(\d+)\.\s*", r"Option \1: ", s)

    # 5. Clean extra newlines & spaces
    s = re.sub(r"\n+", ". ", s)
...
async def elevenlabs_stream(text: str, ws: WebSocket, turn_id: str) -> None:
    if not ELEVENLABS_API_KEY:
        raise RuntimeError("ELEVENLABS_API_KEY is missing from environment configuration.")
        ...
        async with client.stream("POST", url, headers=headers, json=payload) as response:
            if response.status_code != 200:
                ...
                try:
                    ...
                    msg = err_json.get("detail", {}).get("message") or err_json.get("detail") or str(body)
                except Exception:
                    msg = str(body)
...
async def handle_utterance(ws: WebSocket, session_id: str, text: str, state: dict[str, Any]) -> None:
    ...
    await ws.send_json({"type": "assistant_thinking", "turn_id": turn_id, "text": "Thinking..."})
    try:
        answer, meta = await asyncio.to_thread(make_voice_answer, text, memory)
        memory.add("assistant", answer)
        ...
        try:
            await elevenlabs_stream(spoken_text, ws, turn_id)
        except Exception as tts_exc:
            await ws.send_json({"type": "tts_error", "turn_id": turn_id, "error": f"ElevenLabs TTS: {tts_exc}"})
...
async def commit_transcript(ws: WebSocket, session_id: str, text: str, state: dict[str, Any]) -> None:
    text = text.strip()
    if not text or text == state.get("committed_transcript"):
        return
    if state.get("turn_busy"):
        return
    state["committed_transcript"] = text
    await ws.send_json({
...
async def schedule_inactivity_commit(ws: WebSocket, session_id: str, state: dict[str, Any], delay: float = 0.45) -> None:
    try:
        await asyncio.sleep(delay)
        if state.get("turn_busy"):
            return
        latest = state.get("latest_transcript", "").strip()
        if latest and latest != state.get("committed_transcript"):
            await commit_transcript(ws, session_id, latest, state)
    except asyncio.CancelledError:
        pass
...
async def google_transcript_reader(ws: WebSocket, google_ws, session_id: str, state: dict[str, Any]) -> None:
    try:
        async for raw in google_ws:
            ...
            # Ignore transcription if assistant is actively answering
            if state.get("turn_busy"):
                continue
            ...
            if interim and interim.get("text"):
                ...
                if text:
                    ...
                    timer = state.get("inactivity_timer")
                    if timer and not timer.done():
                        timer.cancel()
                    state["inactivity_timer"] = asyncio.create_task(
                        schedule_inactivity_commit(ws, session_id, state, delay=0.45)
                    )
            ...
            if final and final.get("text"):
                ...
                if text:
                    ...
                    if timer and not timer.done():
                        timer.cancel()
                    await commit_transcript(ws, session_id, text, state)
    ...
    try:
        ...
        while True:
            ...
            if message.get("text"):
                ...
                if event.get("type") in ("speech_start", "interrupt"):
                    ...
                    if latest and latest != state.get("committed_transcript"):
                        ...
                        if timer and not timer.done():
                            timer.cancel()
                        state["inactivity_timer"] = asyncio.create_task(
                            schedule_inactivity_commit(ws, session_id, state, delay=0.2)
                        )
                    try:
