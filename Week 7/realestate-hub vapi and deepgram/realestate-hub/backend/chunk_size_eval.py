...
from config import EMBEDDING_BATCH_DELAY_SECONDS, VECTORSTORE_DIR, get_embeddings
...
if __name__ == "__main__":
    ...
    for idx, cs in enumerate(chunk_sizes):
        ...
        store = Chroma(persist_directory=str(VECTORSTORE_DIR / f"chroma_eval_{cs}"), collection_name=f"eval_{cs}",
        ...
        if idx < len(chunk_sizes) - 1:
            print(f"Pausing {EMBEDDING_BATCH_DELAY_SECONDS:.0f}s before next chunk size (rate-limit safety buffer)...")
