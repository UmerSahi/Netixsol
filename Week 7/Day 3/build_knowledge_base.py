...
from config import get_engine, DB_BACKEND, PROJECT_ROOT, resolve_project_path

random.seed(11)
SOURCE_CSV = resolve_project_path(sys.argv[1]) if len(sys.argv) > 1 else PROJECT_ROOT / "Property_with_Feature_Engineering.csv"
...
def build_properties():
    ...
    final.to_csv(PROJECT_ROOT / "properties.csv", index=False)
    ...
    agents_df.to_csv(PROJECT_ROOT / "agents.csv", index=False)
    return final
...
def build_locations_and_kb(props):
    ...
    locations_df.to_csv(PROJECT_ROOT / "locations.csv", index=False)
    ...
    pd.DataFrame(amenities_rows).to_csv(PROJECT_ROOT / "amenities.csv", index=False)
    ...
    pd.DataFrame(schools_rows, columns=["locality_full", "school_name", "level", "distance_km_est"]).to_csv(PROJECT_ROOT / "schools.csv", index=False)
    ...
    pd.DataFrame(hospitals_rows, columns=["locality_full", "hospital_name", "specialty", "distance_km_est"]).to_csv(PROJECT_ROOT / "hospitals.csv", index=False)
    ...
    pd.DataFrame(dev_rows).to_csv(PROJECT_ROOT / "developers.csv", index=False)
    ...
    pd.DataFrame(payment_plans_rows).to_csv(PROJECT_ROOT / "payment_plans.csv", index=False)
    ...
    pd.DataFrame(faqs_rows, columns=["question", "category", "answer"]).to_csv(PROJECT_ROOT / "faqs.csv", index=False)
...
def load_into_database():
    ...
    with engine.begin() as conn:
        for t in tables:
            pd.read_csv(PROJECT_ROOT / (t + ".csv")).to_sql(t, conn, if_exists="replace", index=False)
